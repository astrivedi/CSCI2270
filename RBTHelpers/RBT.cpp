...
// Implement all of the methods declared in the RBT class using recursive procedures. 
...
*/
void RBT::prettyPrint(Node* T, int space)
{
  // Base case
  if (T == 0) return;
...
void RBT::insert(Node* &T, Node* p, int val) {
  if (T == 0) { // Tree is empty 
    ...
    T = node;
    // insert the code to assign parent pointer 
    T->parent = p;
    return;
  }
  else {
...
void RBT::rotateLeft(Node* &T) {
  if (T == 0) return;
    ...
    Node* new_right = new_root->left; // could be 0 .. 
    T->right = new_right;
    if (new_right != 0) {
      //set its parent pointer properly ...
      new_right->parent = T;
    }
    new_root->left = T;
...
void RBT::rotateRight(Node* &T) {
   if (T == 0) return;
    ...
    Node* new_left = new_root->right; // could be 0 .. 
    T->left = new_left;
    if (new_left != 0) {
      //set its parent pointer properly ...
      new_left->parent = T;
